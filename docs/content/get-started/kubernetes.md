...
- [cert-manager](https://cert-manager.io/) — **only** if you set the admission
  ...
  see [Production](/running/production/#admission-webhook-tls) for the
  trade-off.
...
Check [Upgrading](/reference/upgrading/)
...
- [Configuration reference](/reference/configuration/) — every binary flag and
  the chart value that drives it.

For production sizing — S3 storage, multi-replica HA, observability, and webhook
hardening — see the [Production guide](/running/production/).
...
- [Quickstart tutorial](/get-started/quickstart/) — five steps from a Helm install
  to a published artifact.
- [Production hardening](/running/production/) — storage, observability, the
