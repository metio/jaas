...
- **stageset-controller producer-aware sources guide:**
  <https://stageset.projects.metio.wtf/rendering/producer-aware-sources/>
...
- [Operator mode](/operator/operator-mode/) — the full operator reference,
  including the `ExternalArtifact` `spec.sourceRef` back-pointer contract that
  producer-aware consumers match on.
- [Snippet sources](/operator/snippet-sources/) — back the manifests with a
