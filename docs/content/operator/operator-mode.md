...
With the [Helm chart](/get-started/) set `operator.enabled: true`:
...
covered in [Jsonnet libraries](/rendering/jsonnet-libraries/).
...
can reach. See [Tenancy and RBAC](/security/tenancy-and-rbac/) for the verbs the
...
- [Snippet sources](/operator/snippet-sources/) — inline files, a Flux `sourceRef`,
  multi-snippet trees, and chaining one snippet's output into another.
- [Jsonnet libraries](/rendering/jsonnet-libraries/) — the `JsonnetLibrary` CRD,
  OCI-mounted shared libraries, and how imports resolve.
- [Tenancy and RBAC](/security/tenancy-and-rbac/) — per-snippet impersonation and
  the tenant ServiceAccount's permissions.
- [Storage and HA](/operator/storage-and-ha/) — the local and S3 backends, leader
