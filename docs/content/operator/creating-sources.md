...
it. [Snippet sources](/operator/snippet-sources/) covers wiring the finished source
...
[Tenancy and RBAC](/security/tenancy-and-rbac/) for the exact verbs.
...
uses IRSA only for its own [S3 storage backend](/operator/storage-and-ha/) — a
