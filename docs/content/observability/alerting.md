...
| `JaaSOperatorUnavailable` | critical | `jaas_operator_available` reads 0 — the process serves, its manager cannot start, nothing reconciles. | `operatorUnavailableDuration` (5m) | [operator-unavailable](/runbooks/operator-unavailable/) |
| `JaaSOperatorFlapping` | warning | The manager is reconciling for less than the configured fraction of a 30-minute window, so it comes up and dies repeatedly. | `operatorAvailabilityRatio` (0.9), `operatorFlappingDuration` (10m) | [operator-unavailable](/runbooks/operator-unavailable/) |
